...
import { ChainId, WETH } from '@acentswap/ade-sdk-trial'
...
export const useAde = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getAdeContract(library.getSigner()), [library])
}
...
export const useAdeVaultContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getAdeVaultContract(library.getSigner()), [library])
}
  ...
  if (chainId) {
    ...
    switch (chainId) {
      ...
      case ChainId.TESTNET:
        address = '0x260fF7E1969F04ad21B39C6e236137a79e7c4C9D' // FIXME
        break
    }
  }
