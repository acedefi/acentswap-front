...
import { Modal, Text, Flex, BalanceInput, Box, Button, PancakeRoundIcon } from '@acentswap/ace-uikit'
...
import { useAde, useFarmAuctionContract } from 'hooks/useContract'
...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient ADE balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% ADE or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <InnerContent>
        ...
        <BalanceInput
          ...
          currencyValue={
            adePriceUsdc.gt(0) &&
            `~${bid ? adePriceUsdc.times(new BigNumber(bid)).toNumber().toLocaleString() : '0.00'} USD`
          }
        />
