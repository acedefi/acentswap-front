...
import { Box, Flex, FlexProps, Skeleton, Text } from '@acentswap/ace-uikit'
import { useTranslation } from 'contexts/Localization'
import { BetPosition, NodeRound, Round } from 'state/types'
import { formatUsdv2, formatAcev2, getRoundPosition, getPriceDifference } from '../../helpers'
import { formatAce, formatUsd } from '../History/helpers'
...
const getPrizePoolAmount = (totalAmount: PrizePoolRowProps['totalAmount']) => {
  ...
  return formatAcev2(totalAmount)
}
...
export const PrizePoolRow: React.FC<PrizePoolRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      ...
      <Text bold>{`${getPrizePoolAmount(totalAmount)} ACE`}</Text>
    </Row>
  )
}
  ...
  return (
    <Row height="18px" {...props}>
      ...
      <Flex alignItems="center">
        ...
        <Text mx="4px">|</Text>
        <Text fontSize="12px" lineHeight="18px">{`${formatAce(amount)} ACE`}</Text>
      </Flex>
    </Row>
  )
}
...
export const PrizePoolHistoryRow: React.FC<PrizePoolHistoryRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      <Text bold>{t('Prize Pool')}:</Text>
      <Text bold>{`${getPrizePoolAmountHistory(totalAmount)} ACE`}</Text>
    </Row>
  )
}
