...
import { Box, Flex, Heading, Text, PrizeIcon, BlockIcon, LinkExternal, useTooltip, InfoIcon } from '@acentswap/ace-uikit'
...
import { formatAce, getNetPayout } from './helpers'
...
const BetResult: React.FC<BetResultProps> = ({ bet, result }) => {
  ...
  const totalPayout = multiplyPriceByAmount(aceUsdcPrice, payout)
    ...
    <>
      ...
      <StyledBetResult>
        ...
        <Flex alignItems="center" justifyContent="space-between" mb="16px">
          ...
          <Text>{`${formatAce(bet.amount)} ACE`}</Text>
        </Flex>
        <Flex alignItems="start" justifyContent="space-between">
          <Text bold>{isWinner ? t('Your winnings') : t('Your Result')}:</Text>
          <Box style={{ textAlign: 'right' }}>
            <Text bold color={getResultColor()}>{`${isWinner ? '+' : '-'}${formatAce(payout)} ACE`}</Text>
          ...
          <>
            ...
            <Flex alignItems="start" justifyContent="space-between">
              ...
              <Flex justifyContent="end">
                <Text fontSize="14px" color="textSubtle">{`${formatAce(returned)} ACE`}</Text>
                <span ref={targetRef}>
