...
import { Box, Flex, Heading, Text, Button, Link, OpenNewIcon } from '@acentswap/ace-uikit'
import { useTranslation } from 'contexts/Localization'
import { getRoundResult, Result } from 'state/predictions/helpers'
import { REWARD_RATE } from 'state/predictions/config'
import { getExplorerLink } from 'utils'
import { multiplyPriceByAmount } from 'utils/prices'
import { useAceUsdcPrice } from 'hooks/useUsdcPrice'
import { useGetCurrentEpoch } from 'state/predictions/hooks'
import { Bet, BetPosition } from 'state/types'
import { formatAce, getMultiplier, getNetPayout } from '../helpers'
...
const PnlTab: React.FC<PnlTabProps> = ({ hasBetHistory, bets }) => {
  ...
  const aceUsdcPrice = useAceUsdcPrice()
  ...
  const netResultInUsd = multiplyPriceByAmount(aceUsdcPrice, netResultAmount)
  const avgBnbWonInUsd = multiplyPriceByAmount(aceUsdcPrice, avgBnbWonPerRound)
  const betRoundInUsd = multiplyPriceByAmount(aceUsdcPrice, summary.won.bestRound.multiplier)
  const avgPositionEnteredInUsd = multiplyPriceByAmount(aceUsdcPrice, avgPositionEntered)
    ...
    <Box p="16px">
      ...
      <Flex>
        ...
        <Flex flexDirection="column" justifyContent="center" pl="24px">
          ...
          <Text bold fontSize="24px" lineHeight="1" color={netResultIsPositive ? 'success' : 'failure'}>
            {`${netResultIsPositive ? '+' : ''}${formatAce(netResultAmount)} ACE`}
          </Text>
      ...
      <Box pl="8px">
        ...
        <Text bold color={avgBnbWonIsPositive ? 'success' : 'failure'}>
          {`${avgBnbWonIsPositive ? '+' : ''}${formatAce(avgBnbWonPerRound)} ACE`}
        </Text>
        <Text small color="textSubtle">
        ...
        {hasBestRound && (
          <>
            ...
            <Flex alignItems="flex-end">
              <Text bold color="success">{`+${formatAce(summary.won.bestRound.payout)} ACE`}</Text>
        ...
        <Text bold>{`${formatAce(avgPositionEntered)} ACE`}</Text>
        <Text small color="textSubtle">
        ...
        <SummaryRow type="won" summary={summary} aceUsdcPrice={aceUsdcPrice} />
        <SummaryRow type="lost" summary={summary} aceUsdcPrice={aceUsdcPrice} />
        <SummaryRow type="entered" summary={summary} aceUsdcPrice={aceUsdcPrice} />
