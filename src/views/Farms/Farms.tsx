...
import { Image, Heading, RowType, Toggle, Text, Button, ArrowForwardIcon, Flex } from '@acentswap/ace-uikit'
import { ChainId } from '@acentswap/ade-sdk'
import styled from 'styled-components'
import FlexLayout from 'components/Layout/Flex'
import Page from 'components/Layout/Page'
import { useFarms, usePollFarmsWithUserData, usePriceAdeUsdc } from 'state/farms/hooks'
...
export const getDisplayApr = (adeRewardsApr?: number, lpRewardsApr?: number) => {
  if (adeRewardsApr && lpRewardsApr) {
    return (adeRewardsApr + lpRewardsApr).toLocaleString('en-US', { maximumFractionDigits: 2 })
  }
  if (adeRewardsApr) {
    return adeRewardsApr.toLocaleString('en-US', { maximumFractionDigits: 2 })
  }
  return null
}
...
const Farms: React.FC = () => {
  ...
  const farmsList = useCallback(
    (farmsToDisplay: DeserializedFarm[]): FarmWithStakedValue[] => {
      let farmsToDisplayWithAPR: FarmWithStakedValue[] = farmsToDisplay.map((farm) => {
        ...
        const { adeRewardsApr, lpRewardsApr } = isActive
          ? getFarmApr(new BigNumber(farm.poolWeight), adePrice, totalLiquidity, getAddress(farm.lpAddresses))
          : { adeRewardsApr: 0, lpRewardsApr: 0 }
  ...
  const rowData = chosenFarmsMemoized.map((farm) => {
    ...
    // const lpLabel = farm.lpSymbol && farm.lpSymbol.split(' ')[0].toUpperCase().replace('ADE', '') // FIXME replace ??
