...
import { getAddress, getAdeVaultAddress } from 'utils/addressHelpers'
import { BIG_ZERO } from 'utils/bigNumber'
import { registerToken } from 'utils/wallet'
import { getBalanceNumber, getFullDisplayBalance } from 'utils/formatBalance'
import { convertSharesToAde, getPoolBlockInfo } from 'views/Pools/helpers'
...
const ActionPanel: React.FC<ActionPanelProps> = ({ account, pool, userDataLoaded, expanded, breakpoints }) => {
  ...
  const adeVaultContractAddress = getAdeVaultAddress()
  ...
  const poolStakingTokenBalance = isAutoVault
    ? adeAsBigNumber.plus(stakingTokenBalance)
  ...
  const getTotalStakedBalance = () => {
    if (isAutoVault) {
      return getBalanceNumber(totalAdeInVault, stakingToken.decimals)
    }
    if (isManualAdePool) {
      const manualAdeTotalMinusAutoVault = new BigNumber(totalStaked).minus(totalAdeInVault)
      return getBalanceNumber(manualAdeTotalMinusAutoVault, stakingToken.decimals)
    }
    ...
    <StyledActionPanel expanded={expanded}>
      <InfoSection>
        ...
        {poolContractAddress && (
          <Flex mb="8px" justifyContent={['flex-end', 'flex-end', 'flex-start']}>
            <LinkExternal
              href={`${BASE_EXPLORER_URLS[chainId]}/address/${isAutoVault ? adeVaultContractAddress : poolContractAddress}`}
              bold={false}
            >
              {t('View Contract')}
            </LinkExternal>
      ...
      <ActionContainer>
        {showSubtitle && (
          <Text mt="4px" mb="16px" color="textSubtle">
            {isAutoVault ? t('Automatic restaking') : `${t('Earn')} ADE ${t('Stake').toLocaleLowerCase()} ADE`}
          </Text>
        )}
        {pool.isAutoVault ? (
