...
import { Card, CardBody, Text, Flex, HelpIcon, Button, Heading, Skeleton, useModal, Box, useTooltip } from '@acentswap/ace-uikit'
...
const BountyCard = () => {
  ...
  const TooltipComponent = ({ fee }: { fee: number }) => (
    <>
      ...
      <Text mb="16px">
        {t(
          'Whenever you successfully claim the bounty, you’re also helping out by activating the Auto ADE Pool’s compounding function for everyone.',
        )}
      </Text>
      <Text style={{ fontWeight: 'bold' }}>
        {t('Auto-Compound Bounty: %fee%% of all Auto ADE pool users pending yield', { fee: fee / 100 })}
      </Text>
    </>
  )
        ...
        <CardBody>
          ...
          <Flex alignItems="center" justifyContent="space-between">
            <Flex flexDirection="column" mr="12px">
              <Heading>
                {hasFetchedAdeBounty ? (
                  <Balance fontSize="20px" bold value={adeBountyToDisplay} decimals={3} />
                ) : (
