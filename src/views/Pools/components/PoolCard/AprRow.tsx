...
import { Flex, TooltipText, IconButton, useModal, CalculateIcon, Skeleton, useTooltip } from '@acentswap/ace-uikit'
...
const AprRow: React.FC<AprRowProps> = ({ pool, stakedBalance, performanceFee = 0 }) => {
  ...
  const tooltipContent = isBlindMode() ? t('Amount of ADE rewards allocated to the Glitter mine participants issued with each Acent block (~5-6s/block). We will convert this value to APY/APR value after Pioneer Farming Mode is completed.') : (isAutoVault
  ...
  const adeFarm = useFarmFromPid(0)
  ...
  return (
    isBlindMode() ? <Flex alignItems="center" justifyContent="space-between">
      ...
      <Flex alignItems="center">
        {adeFarm && adeFarm.poolWeight ? `${(ADE_PER_BLOCK * adeFarm.poolWeight.toNumber()).toFixed(2)}/ Block` : ''}
      </Flex>
