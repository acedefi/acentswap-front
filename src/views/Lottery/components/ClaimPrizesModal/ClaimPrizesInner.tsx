...
import { Flex, Button, Text, AutoRenewIcon, PresentWonIcon } from '@acentswap/ace-uikit'
...
const ClaimInnerContainer: React.FC<ClaimInnerProps> = ({ onSuccess, roundsToClaim }) => {
  ...
  const adeReward = activeClaimData.cakeTotal
  const dollarReward = adeReward.times(adePriceUsdc)
  const rewardAsBalance = getBalanceAmount(adeReward).toNumber()
