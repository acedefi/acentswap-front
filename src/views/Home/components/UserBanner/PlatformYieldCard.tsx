...
import { Card, CardBody, Flex, Skeleton, Text } from '@acentswap/ace-uikit'
...
const PlatformYieldCard = () => {
  ...
  useEffect(() => {
    const fetchHarvestedData = async () => {
      ...
      if (harvested?.totalClaimAmount || harvested?.totalADEVaultProfit) {
        setHarvestedSum(Number(harvested.totalClaimAmount) + Number(harvested.totalADEVaultProfit))
      }
    }
    if (account) {
      fetchHarvestedData()
  ...
  const totalPlatformYield = (getBalanceNumber(new BigNumber(harvestedSum), tokens.ade.decimals) + farmEarningsSum) * adePriceUsdc.toNumber()
