...
import { AutoRenewIcon, Button, Card, CardBody, Flex, Skeleton, Text, Link, ArrowForwardIcon } from '@acentswap/ace-uikit'
...
const HarvestCard = () => {
  ...
  // const adePriceUsdc = new BigNumber(0) // XXX debug hard code for landing banner
  const earningsUsdc = new BigNumber(farmEarningsSum).multipliedBy(adePriceUsdc)
  ...
  const hasAdePoolToCollect = numTotalToCollect - numFarmsToCollect > 0

  const earningsText = getEarningsText(numFarmsToCollect, hasAdePoolToCollect, earningsUsdc, t)
  ...
  const harvestAllFarms = useCallback(async () => {
    ...
    for (const farmWithBalance of farmsWithStakedBalance) {
      try {
        ...
        toastSuccess(
          `${t('Harvested')}!`,
          t('Your %symbol% earnings have been sent to your wallet!', { symbol: 'ADE' }),
        )
      } catch (error) {
