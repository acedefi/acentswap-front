...
import { Flex, Heading, Text, Button, Link, useMatchBreakpoints } from '@acentswap/ace-uikit'
import { useTranslation } from 'contexts/Localization'
import { isCountdown } from 'utils'

import { StyledBigHeading } from '../StyledHeadings'
import { adeTokenIntroData } from './data'
...
const diamondImages: CompositeImageProps = {
  path: `${process.env.PUBLIC_URL}/images/home/trade/`,
  attributes: [
    {
      src: 'CAKE',
      alt: 'CAKE',
...
const AdeTokenIntro = () => {
  ...
  const { headingText, items } = adeTokenIntroData
              ...
              >
                <Flex flexDirection={['column', 'column', 'column', 'row']} mb="10px">
                  ...
                  <Text style={{ alignSelf: 'center', fontWeight: 'bold' }} height="30px" lineHeight="34px" width="100%">
                    {/* <img
                      src="/images/home/ade-info/aquaVector.svg"
                      alt="star1"
                      height="18px"
                      width="18px"
                      style={{
          ...
          <Flex justifyContent={isDesktop ? 'unset' : 'center'}>
            {
              isCountdown() ? (
                <>
                  ...
                  <>
                    <Link external href="https://docs.acentswap.acent.tech/litepaper">
                      <StyledButton mr={10}>{t('White Paper')}</StyledButton>
                    </Link>
                    <Link external href="https://docs.acentswap.acent.tech/litepaper/token-economics">
                      <StyledButton variant="secondary">{t('ADE Tokenomics')}</StyledButton>
                    </Link>
                  </>
              )
            }
