...
import { Flex, Text, Skeleton, Link, Button, ArrowForwardIcon } from '@acentswap/ace-uikit'
...
const LotteryCardContent = () => {
  ...
  const adePriceUsdcAsString = usePriceAdeUsdc().toString()

  const adePrizesText = t('%cakePrizeInUsd% in ADE prizes this round', { cakePrizeInUsd: adePriceUsdcAsString })
  const [pretext, prizesThisRound] = adePrizesText.split(adePriceUsdcAsString)

  const adePriceUsdc = useMemo(() => {
    return new BigNumber(adePriceUsdcAsString)
  }, [adePriceUsdcAsString])
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      ...
      const prizeInUsdc = adePriceUsdc.times(amountCollectedInCake)
  ...
  }, [lotteryId, slowRefresh, setCurrentLotteryPrize, adePriceUsdc])
