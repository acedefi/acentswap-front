...
export const transformBetResponse = (betResponse: BetResponse): Bet => {
  const bet = {
    ...
    claimedACE: betResponse.claimedACE ? parseFloat(betResponse.claimedACE) : 0,
    claimedNetACE: betResponse.claimedNetACE ? parseFloat(betResponse.claimedNetACE) : 0,
...
export const transformUserResponse = (userResponse: UserResponse): PredictionUser => {
  ...
  return {
    ...
    totalACE: totalACE ? parseFloat(totalACE) : 0,
    totalACEBull: totalACEBull ? parseFloat(totalACEBull) : 0,
    totalACEBear: totalACEBear ? parseFloat(totalACEBear) : 0,
    totalBetsClaimed: numberOrNull(totalBetsClaimed),
    totalACEClaimed: totalACEClaimed ? parseFloat(totalACEClaimed) : 0,
    winRate: winRate ? parseFloat(winRate) : 0,
    averageACE: averageACE ? parseFloat(averageACE) : 0,
    netACE: netACE ? parseFloat(netACE) : 0,
  }
}
...
export const getTotalWon = async (): Promise<number> => {
  ...
  const totalACE = market.totalACE ? parseFloat(market.totalACE) : 0
  const totalACETreasury = market.totalACETreasury ? parseFloat(market.totalACETreasury) : 0

  return Math.max(totalACE - totalACETreasury, 0)
}
