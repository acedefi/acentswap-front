...
const formatAdeFactoryResponse = (rawAdeFactory?: AdeFactory) => {
  if (rawAdeFactory) {
    return {
      totalTransactions: parseFloat(rawAdeFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawAdeFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawAdeFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatAdeFactoryResponse(data?.adeFactories?.[0])

      // When ADE is first launched, both 24 and 48 hr will be null
      const overviewData24Formatted = formatAdeFactoryResponse(data24?.adeFactories?.[0])
      const overviewData24 = overviewData24Formatted || overviewData
      const overviewData48Formatted = formatAdeFactoryResponse(data48?.adeFactories?.[0]) // FIXME hard code no data for 48h
