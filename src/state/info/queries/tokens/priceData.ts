  ...
  try {
    ...
    Object.keys(prices).forEach((priceKey) => {
      ...
      if (timestamp) {
        tokenPrices.push({
          timestamp,
          derivedACE: prices[priceKey]?.derivedACE ? parseFloat(prices[priceKey].derivedACE) : 0,
          priceUSD: 0,
        })
      }
    })
    ...
    Object.keys(prices).forEach((priceKey) => {
      ...
      if (timestamp) {
        ...
        if (tokenPriceIndex >= 0) {
          const { derivedACE } = tokenPrices[tokenPriceIndex]
          tokenPrices[tokenPriceIndex].priceUSD = parseFloat(prices[priceKey]?.acePrice ?? 0) * derivedACE
        }
      }
    })
