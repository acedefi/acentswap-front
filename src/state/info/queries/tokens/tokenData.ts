...
import { useAcePrices } from 'views/Info/hooks/useAcePrices'

interface TokenFields {
  id: string
  symbol: string
  name: string
  derivedACE: string // Price in ACE per token
...
interface FormattedTokenFields
  extends Omit<TokenFields, 'derivedACE' | 'derivedUSD' | 'tradeVolumeUSD' | 'totalTransactions' | 'totalLiquidity'> {
  derivedACE: number
  derivedUSD: number
...
const parseTokenData = (tokens?: TokenFields[]) => {
  ...
  return tokens.reduce((accum: { [address: string]: FormattedTokenFields }, tokenData) => {
    const { derivedACE, derivedUSD, tradeVolumeUSD, totalTransactions, totalLiquidity } = tokenData
    accum[tokenData.id] = {
      ...tokenData,
      derivedACE: parseFloat(derivedACE),
    ...
    const fetch = async () => {
      ...
      if (error) {
        ...
        const formatted = tokenAddresses.reduce((accum: { [address: string]: TokenData }, address) => {
          ...
          const priceUSD = current ? current.derivedACE * croPrices.current : 0
          const priceUSDOneDay = oneDay ? oneDay.derivedACE * croPrices.oneDay : 0
          // const priceUSDWeek = week ? week.derivedACE * croPrices.week : 0
          const priceUSDWeek = week ? week.derivedACE * (croPrices.week ?? croPrices.oneDay) : 0 // FIXME hard code no data for 7d
