...
import { Token } from '@acentswap/ade-sdk-trial'
...
export const usePriceAdeUsdc = (): BigNumber => {
  // const adeAceFarm = useFarmFromPid(3) // FIXME pid for ADE/WACE
  const adeAceFarm = useFarmFromLpPair(tokens.ade, tokens.wace) // FIXME pid for ADE/WACE

  const adePriceUsdcAsString = adeAceFarm.tokenPriceUsdc
  ...
  // return prices[serializeTokens().ade.address] ?? new BigNumber(0);
}
